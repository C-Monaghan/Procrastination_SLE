---
title: "Procrastination and Subjective Life Expectancy"
...
A number of reprocessing steps were performed to clean and prepare the dataset for analysis. These transformations included renaming variables for clarity, recoding categorical variables, and creating age groups to facilitate comparisons across demographic categories.
...
-   A positive correlation between procrastination and a consideration for immediate consequences @fig-correlation-1
-   Negative correlation between age and procrastination @fig-correlation-2
...
#### Procrastination and Immediate Consequences
...
data_classification %>%
  ...
  labs(
    title = "Latent Profile Plot of Item Response Probabilities",
...
After performing the LCA we ran a **multi-group structural equation model (MSEM)** to explore the relationships between both latent constructs and observed variables for each class membership.
...
data_sem <- data_classification %>%
  mutate(
    ...
    Total_CFCI = rowSums(across(c(I_1:I_7)), na.rm = TRUE),
    Total_CFCF = rowSums(across(c(F_1:F_7)), na.rm = TRUE),
    ...
    # 0 = no presence of illness // 1 = presence of illness
    ChronicIllness = ifelse(ChronicIllness == 0, 1, 0),
    ChronicIllness = factor(ChronicIllness, levels = c(0, 1)),
    )
...
model <- '
  ...
  # RESIDUALS - (Monaghan et al., 2024)
  PPS1 ~~ PPS2;
  PPS10 ~~ PPS11;
  
  # STRUCTURAL MODEL
  Proc ~ SLE + SLE:Age + Age + Sex + ChronicIllness;
'

# Fitting and outputting model
fit <- sem(model = model, data = data_sem, estimator = "ML", missing = "fiml.x", group = "Class")
